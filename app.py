...
# ============================================
# GESTIÓN DE PROVEEDORES (CON GROQ Y FALLBACK FORZADO)
# ============================================
...
def get_provider_config():
    ...
    # Log para ver qué proveedores están disponibles
    print(f"📋 Proveedores disponibles: {list(providers.keys())}", file=sys.stderr)
    sys.stderr.flush()
    
    return providers

# ============================================
# FUNCIÓN DE LLAMADA CON FALLBACK INTELIGENTE
# ============================================
...
def llamar_modelo(pregunta, proveedores_preferidos=None):
    ...
    # Si no se especifican preferencias, usar el orden de prioridad
    if proveedores_preferidos is None:
        proveedores_preferidos = sorted(providers.keys(), key=lambda p: providers[p]['priority'])
    
    # Añadir Groq al final como fallback (por si no está en la lista)
    ...
    for provider_name in proveedores_preferidos:
        ...
        try:
            ...
            ultimo_error = e
            sys.stderr.flush()
            # Si DeepSeek falla por saldo, forzar intento con Groq (si no se ha intentado ya)
            if provider_name == 'deepseek' and '402' in str(e) and 'groq' in providers and 'groq' not in proveedores_preferidos:
                proveedores_preferidos.append('groq')
            continue
