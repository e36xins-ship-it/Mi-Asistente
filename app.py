...
Arquitectura: Flask + PostgreSQL (Neon) + Multi-LLM + Task Queue + Memory
"""
...
# Límites y seguridad
INTERVALO_APRENDIZAJE = 600          # 10 minutos entre ciclos
TIEMPO_MAXIMO_TAREA = 600            # 10 minutos máximo por microtarea
MAX_REINTENTOS_TAREA = 3             # Reintentos antes de marcar como fallida
BACKUP_DIR = "backups"
REPO_DIR = "repo"
...
def get_db_connection():
    """Retorna una conexión a PostgreSQL con soporte para diccionarios."""
...
def init_db():
    ...
    # Tabla de tareas (cola principal)
    cur.execute("""
    ...
    # Tabla de configuración (claves API, etc.)
    cur.execute("""
    ...
    # Tabla de memoria episódica (experiencias)
    cur.execute("""
    ...
    # Tabla de memoria semántica (conocimiento general)
    cur.execute("""
    ...
    # Tabla de checkpoints (para rollback)
    cur.execute("""
    ...
    # Tabla de fallos (registro de errores)
    cur.execute("""
    ...
    # Tabla de cola de microtareas (para auto-mejora)
    cur.execute("""
...
def db_add_tarea(descripcion, prioridad=0):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO tareas (descripcion, prioridad) VALUES (%s, %s) RETURNING id",
...
def db_marcar_completada(tarea_id, error=None):
    ...
    if error:
        cur.execute(
            "UPDATE tareas SET estado = 'fallida', completada_en = CURRENT_TIMESTAMP, intentos = intentos + 1, ultimo_error = %s WHERE id = %s",
            (error, tarea_id)
        )
    else:
        cur.execute(
            "UPDATE tareas SET estado = 'completada', completada_en = CURRENT_TIMESTAMP WHERE id = %s",
            (tarea_id,)
        )
    conn.commit()
    cur.close()
...
def db_get_microtarea_pendiente():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM microtareas WHERE estado = 'pendiente' ORDER BY prioridad DESC, creada_en LIMIT 1"
    )
    result = cur.fetchone()
    cur.close()
    conn.close()
...
def db_add_microtarea(descripcion, estimado_minutos=5, prioridad=0):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO microtareas (descripcion, estimado_minutos, prioridad) VALUES (%s, %s, %s) RETURNING id",
        (descripcion, estimado_minutos, prioridad)
    )
...
def db_microtarea_iniciada(id):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE microtareas SET estado = 'en_progreso', iniciada_en = CURRENT_TIMESTAMP, intentos = intentos + 1 WHERE id = %s",
        (id,)
    )
    conn.commit()
    cur.close()
...
def db_microtarea_completada(id, exito=True, error=None):
    ...
    if exito:
        cur.execute(
            "UPDATE microtareas SET estado = 'completada', completada_en = CURRENT_TIMESTAMP WHERE id = %s",
            (id,)
        )
    else:
        cur.execute(
            "UPDATE microtareas SET estado = 'fallida', completada_en = CURRENT_TIMESTAMP, ultimo_error = %s WHERE id = %s",
            (error, id)
        )
    conn.commit()
    cur.close()
...
def db_guardar_historial(mejora, backup_path, estado="aplicada"):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO historial (mejora, backup_path, estado) VALUES (%s, %s, %s) RETURNING id",
        (mejora[:500], backup_path, estado)
    )
...
def db_guardar_checkpoint(descripcion, codigo):
    ...
    cur.execute(
        "INSERT INTO checkpoints (descripcion, codigo, activo) VALUES (%s, %s, TRUE) RETURNING id",
        (descripcion, codigo)
    )
...
def db_rollback_checkpoint(checkpoint_id):
    ...
    if not row:
        ...
        return None
    codigo = row["codigo"]
    # Desactivar el checkpoint restaurado
...
def db_registrar_fallo(tarea_id, error, stack_trace):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO fallos (tarea_id, error, stack_trace) VALUES (%s, %s, %s)",
        (tarea_id, error[:500], stack_trace[:1000])
    )
    conn.commit()
    cur.close()
    conn.close()
...
def db_guardar_memoria_episodica(pregunta, respuesta, leccion, exito=True):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO memoria_episodica (pregunta, respuesta, leccion, exito) VALUES (%s, %s, %s, %s)",
        (pregunta, respuesta[:500], leccion[:500], exito)
    )
    conn.commit()
    cur.close()
    conn.close()
...
def db_guardar_memoria_semantica(key, value):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO memoria_semantica (key, value) VALUES (%s, %s) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, timestamp = CURRENT_TIMESTAMP",
        (key, value)
    )
    conn.commit()
    cur.close()
...
def get_provider_config():
    """Carga configuración de proveedores desde DB o variables de entorno."""
...
def llamar_modelo(pregunta, proveedores_preferidos=None):
    ...
    # Asegurar que Groq esté al final como fallback
    ...
    for provider_name in proveedores_preferidos:
        ...
        try:
            ...
            ultimo_error = e
            sys.stderr.flush()
            # Si DeepSeek falla por saldo, forzar intento con Groq
...
def validar_sintaxis(codigo: str) -> Tuple[bool, str]:
    """Valida que el código sea sintácticamente correcto usando ast."""
    try:
        ast.parse(codigo)
...
def validar_semantica(codigo: str, timeout: int = 10) -> Tuple[bool, str]:
    """
    Valida que el código sea semánticamente correcto.
    Intenta compilarlo y ejecuta pruebas básicas en un sandbox.
    """
    try:
        # Compilar el código
        compile(codigo, '<string>', 'exec')
        
        # Ejecutar en un entorno aislado (sandbox básico)
...
def validar_codigo_completo(codigo: str) -> Tuple[bool, str, Optional[str]]:
    """
    Validación completa: sintaxis + semántica.
    Retorna (éxito, mensaje, código_corregido_sugerido).
    """
    # Validar sintaxis
...
def guardar_checkpoint(descripcion: str = "") -> int:
    """Guarda un checkpoint del código actual en la base de datos."""
...
def restaurar_checkpoint(checkpoint_id: int = None) -> bool:
    """Restaura un checkpoint desde la base de datos."""
    ...
    # Validar el código antes de restaurar
...
def generar_mejora(objetivo: str, historial: List[Dict]) -> Optional[str]:
    """
    Genera una mejora de código usando el modelo configurado.
    """
    prompt = f"""
    ...
    Mejoras anteriores:
    {json.dumps(historial[-5:], indent=2)}
...
def extraer_codigo(respuesta: str) -> Optional[str]:
    """Extrae el bloque de código Python de la respuesta del modelo."""
    patron = r"```python\n(.*?)```"
    match = re.search(patron, respuesta, re.DOTALL)
    if match:
        return match.group(1).strip()
    
    # Intentar detectar código sin marcadores
...
def aplicar_mejora(codigo: str) -> Tuple[bool, str]:
    """
    Aplica una mejora después de validarla.
...
def ejecutar_microtarea(microtarea: Dict) -> bool:
    """
    Ejecuta una microtarea específica.
...
def ciclo_aprendizaje():
    ...
    Se ejecuta cada INTERVALO_APRENDIZAJE segundos.
    """
    print("🧠 Iniciando ciclo de aprendizaje...", file=sys.stderr)
    
    # 1. Verificar microtareas pendientes
    ...
    Contexto de memoria reciente:
    {json.dumps(memoria, indent=2)}
    ...
    try:
        ...
        if codigo:
            exito, mensaje = aplicar_mejora(codigo)
            if exito:
                db_guardar_memoria_episodica(
                    "Aprendizaje libre",
                    respuesta[:500],
                    f"Mejora aplicada: {mensaje}",
                    exito=True
                )
                print(f"✅ Mejora libre aplicada: {mensaje}", file=sys.stderr)
            else:
                db_guardar_memoria_episodica(
                    "Aprendizaje libre fallido",
                    respuesta[:500],
                    f"Error: {mensaje}",
                    exito=False
                )
...
def bucle_aprendizaje():
    """Bucle principal que ejecuta el ciclo de aprendizaje periódicamente."""
...
def git_commit_and_push():
    ...
    # Copiar archivo actual al repositorio
...
def aprender_manual():
    """Activa el ciclo de aprendizaje manualmente."""
    def wrapper():
        ciclo_aprendizaje()
...
def crear_microtarea():
    """Añade una microtarea manualmente."""
    data = request.get_json()
    if not data or 'descripcion' not in data:
        return jsonify({"error": "Falta descripción"}), 400
    microtarea_id = db_add_microtarea(
        data['descripcion'],
        data.get('estimado_minutos', 5),
        data.get('prioridad', 0)
    )
...
def rollback_manual():
    """Restaura el último checkpoint activo."""
    checkpoint = db_get_checkpoint_activo()
    if not checkpoint:
        return jsonify({"error": "No hay checkpoint disponible"}), 400
...
if __name__ == "__main__":
    print("🚀 Iniciando Aether (Asistente Autónomo)", file=sys.stderr)
    ...
    # Iniciar bucle de aprendizaje en segundo plano
